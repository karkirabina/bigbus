						...
						resp = JSON.parse(resp)
							if(Object.keys(resp).length > 0){
								...
								Object.keys(resp).map(k=>{
									...
									// var availableSeats = parseInt(resp[k].availability) - parseInt(resp[k].qty);
									// tr.append('<td>' + availableSeats + '</td>');
									var availability = parseInt(resp[k].availability);
				var qty = parseInt(resp[k].qty);
				var availableSeats;

				if (isNaN(availability) || isNaN(qty)) {
				availableSeats = "N/A"; // or any other default value you want to use
				} else {
				availableSeats = availability - qty;
				}
				tr.append('<td>' + availableSeats + '</td>');
